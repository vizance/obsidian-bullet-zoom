...
桌面版和手機版都使用同一個 repo。若手機的 Vault 已透過 Obsidian Sync 同步設定，也可以直接在手機的 BRAT 加入同一個路徑。`0.1.2` 尚未建立 GitHub Release，因此 BRAT 目前仍會取得 `0.1.1`；實體手機更新與複驗完成前，不宣稱手機 UX 已正式通過。
