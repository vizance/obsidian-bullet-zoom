...
- 桌面版人工驗收：`0.1.15` 已於 2026-08-13 在 Obsidian `1.13.7` 專用 `.test-vault` 驗證選單式切換與三層行尾 `↖` 會依序返回 Child、Parent、全文；正式 Vault 仍維持已安裝的 `0.1.8`
...
桌面版和手機版都使用同一個 repo。若手機的 Vault 已透過 Obsidian Sync 同步設定，也可以直接在手機的 BRAT 加入同一個路徑。目前最新公開 GitHub Release 是 `0.1.15`，可直接透過 BRAT 更新。實體手機更新與複驗完成前，不宣稱手機 UX 已正式通過。
