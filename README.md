...
- 桌面版：點擊 Bullet 文字行尾的淡色 `↘`，或點擊普通 Bullet 的圓點。
- 手機與平板：點擊普通 Bullet 的圓點。圓點旁的有限空白區也能點擊，但文字與行尾仍保留給游標操作。
...
- 桌面版：點擊目前聚焦根節點行尾的 `↖`，一次返回一個父層。
- 手機與平板：再次點擊目前聚焦根節點的 Bullet 圓點，一次返回一個父層。
...
- 左側收合箭頭：收合／展開內容。
- 桌面行尾 `↘`、Bullet 圓點或聚焦命令：進入 Zoom。
- 手機與平板再次點擊目前節點的 Bullet 圓點：返回上一層。
...
- 開啟：桌面持續顯示 `↘`／`↖`。
- 關閉：桌面只在滑鼠移到該行或按鈕取得鍵盤焦點時顯示。

手機與平板固定使用左側 Bullet 圓點 Zoom，不會顯示插件的行尾 `↘`／`↖`。
...
這是預期行為。Obsidian 的收合箭頭只負責 fold／unfold。桌面版請點擊行尾 `↘` 或 Bullet 圓點；手機與平板請點擊 Bullet 圓點。你也可以使用聚焦命令。

### 看不到行尾箭頭

手機與平板本來就不顯示插件的行尾箭頭，請改點 Bullet 圓點。桌面版可到「設定 → Bullet Zoom」檢查「永遠顯示行尾縮放箭頭」；如果保持關閉，需要把滑鼠移到該 Bullet 行。
