...
describe('mobile-compatible plugin bundle contract', () => {
	it('declares desktop and mobile support in the manifest', () => {
		...
		expect(manifest.version).toBe('1.28.4');
	});
	...
	it('keeps patch-version metadata aligned', () => {
		...
		expect(packageManifest.version).toBe('1.28.4');
		expect(packageLock.version).toBe('1.28.4');
		expect(packageLock.packages?.['']?.version).toBe('1.28.4');
...
describe('slot editor contract (1.19.0)', () => {
	it('lays a slot row out on one line with a stretching command picker', () => {
		...
		const command = rules.find(
			(rule) => rule.selectorText === 'button.bullet-zoom-slot-command',
		);
...
describe('branch drag styling contract (1.28.0)', () => {
	...
	it('hides the caret and text selection while a drag is active', () => {
		const stylesheet = readProjectFile('styles.css');
		const block = stylesheet.slice(
			stylesheet.indexOf('.bullet-zoom-branch-drag-active .cm-content'),
		);
