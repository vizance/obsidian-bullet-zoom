...
describe('mobile-compatible plugin bundle contract', () => {
	it('declares desktop and mobile support in the manifest', () => {
		...
		expect(manifest.version).toBe('1.29.3');
	});
	...
	it('keeps patch-version metadata aligned', () => {
		...
		expect(packageManifest.version).toBe('1.29.3');
		expect(packageLock.version).toBe('1.29.3');
		expect(packageLock.packages?.['']?.version).toBe('1.29.3');
