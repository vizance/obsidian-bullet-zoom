...
describe('mobile-compatible plugin bundle contract', () => {
	it('declares desktop and mobile support in the manifest', () => {
		...
		expect(manifest.version).toBe('0.1.42');
	});
	...
	it('keeps patch-version metadata aligned', () => {
		...
		expect(packageManifest.version).toBe('0.1.42');
		expect(packageLock.version).toBe('0.1.42');
		expect(packageLock.packages?.['']?.version).toBe('0.1.42');
