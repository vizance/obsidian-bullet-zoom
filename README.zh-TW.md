...
選單開啟期間編輯器會被凍結：文字游標隱藏且無法被拖動，開啟選單的那根手指只會操作選單；關閉後游標與鍵盤回到長按前的狀態。開啟選單時不會讓編輯器取得焦點，因此鍵盤不會被叫出來；版面也以實際可視區域計算，就算鍵盤已經在畫面上，扇形仍會落在鍵盤上方。格子會以短暫的擴散動畫出現，系統設定為減少動態時則不播放。
