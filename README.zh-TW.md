...
上下移動決定要放進哪兩行之間，左右移動決定要放到第幾層。正在搬動的那幾行會染上顏色，包含底下的子項目，讓你看清楚跟著走的是哪些內容。落點則由一條實心橫條標示：它停在你瞄準的間隙，起點對齊該層級的項目實際的位置，所以橫條前面那段空白就是在告訴你會放進第幾層。往右拖會變成上一行的子項目，往左拖則會退回外層。
