...
抓住項目符號，就能把整個分支拖到別的位置。條列與編號清單一視同仁——只要 Bullet Zoom 認得那個標記就能拖，所以編號項目跟著 **Zoom numbered items** 設定走。

滑鼠按住標記後移動約 12 像素開始拖曳；觸控則是按住約三分之一秒。長按只有在「點一下就會開啟項目選單」時才用來拖曳，也就是預設狀態；如果你把點擊改成 Zoom，長按會繼續開選單、觸控拖曳自動停用，選單不會失去唯一的入口。沒有變成拖曳的按壓，行為跟以前完全一樣。

### 落點與階層

上下移動決定要放進哪兩行之間，左右移動決定要放到第幾層。放開之前會有一條線告訴你結果：它停在你瞄準的間隙，左緣就是分支即將採用的縮排。往右拖會變成上一行的子項目，往左拖則會退回外層。
