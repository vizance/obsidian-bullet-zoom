...
語音轉文字或 AI 寫作工具常會插入破壞清單格式的段落。Bullet Zoom 會讓這些內容照常顯示，並在你停止輸入約半秒後整理聚焦區域：每一行都變成 Bullet，縮排到它前面那個 Bullet 的下一層，同一批的行彼此平輩，空行直接移除，文字一個字都不會被改寫。原本就巢狀在更深層的 Bullet 維持不變，程式碼區塊不動。按一次 Undo 只還原這個修復。

同樣的整理在沒有 Zoom 時也會運作。在一般筆記的清單裡口述或貼上內容時，新的行會變成上方清單項目底下的 Bullet。非聚焦時規則更保守：既有的清單項目一律不重新縮排，而且只有在編輯位置上方最近的一行是清單項目時才會動作，一般段落完全不受影響。可在 **Focus page** 區塊的 **Fix broken bullets** 關閉。
